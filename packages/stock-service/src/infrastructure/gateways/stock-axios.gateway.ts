...
import { AxiosInterceptor, Stock, StockInterface } from 'ubialimv-common';

import { StockGatewayInterface } from '../../domain/stock/gateways/stock.gateway.interface';

export default class StockAxiosGateway
  extends AxiosInterceptor
  implements StockGatewayInterface
{
  ...
  async getStock(symbol: string): Promise<Stock | null> {
    const response: { symbols: StockInterface[] } = await this.client.get(
      `q/l/?s=${symbol}&f=sd2t2ohlcvn&h&e=json`,
    );
