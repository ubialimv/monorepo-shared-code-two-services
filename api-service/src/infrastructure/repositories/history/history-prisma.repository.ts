...
export default class HistoryPrismaRepository implements HistoryRepositoryInterface {
    async create(history: historyEntity): Promise<PersistedHistoryEntity> {
        const historyCreated = await prismaHelper.history.create({ data: history.toPlain() });
        return modelToEntity(historyCreated);
    }

    async find(userId: number): Promise<PersistedHistory[]> {
        const historiesFound = await prismaHelper.history.findMany({ where: { userId } });
